...
function setup_get_remote_url() {
    ...
    if (stripos($_SERVER['SERVER_SOFTWARE'], 'apache') !== false) {
        ...
        if (isset($_SERVER['ORIG_PATH_INFO']) && (php_sapi_name() === 'fpm-fcgi') && isset($_SERVER['SCRIPT_NAME'])) {
            $pathinfodec = rawurldecode($_SERVER['ORIG_PATH_INFO']);
            $lenneedle = strlen($pathinfodec);
            // Checks whether SCRIPT_NAME ends with ORIG_PATH_INFO, URL-decoded.
            if (substr($_SERVER['SCRIPT_NAME'], -$lenneedle) === $pathinfodec) {
                ...
                // => SCRIPT_NAME contains 'slash arguments' data too, which is wrongly exposed via ORIG_PATH_INFO as URL-encoded.
                // Fix both $_SERVER['ORIG_PATH_INFO'] and $_SERVER['SCRIPT_NAME'].
                $lenhaystack = strlen($_SERVER['SCRIPT_NAME']);
                $pos = $lenhaystack - $lenneedle;
                // Here $pos is greater than 0 but let's double check it.
                if ($pos > 0) {
                    $_SERVER['ORIG_PATH_INFO'] = $pathinfodec;
        ...
        // NOTE: we should ignore ORIG_PATH_INFO because it is incorrectly encoded using 8bit filesystem legacy encoding in IIS.
        ...
        if (isset($_SERVER['ORIG_PATH_INFO']) and $_SERVER['ORIG_PATH_INFO'] !== '') {
            // Check that ORIG_PATH_INFO works == must not contain the script name.
            if (strpos($_SERVER['ORIG_PATH_INFO'], $_SERVER['SCRIPT_NAME']) === false) {
                $rurl['fullpath'] .= clean_param(urldecode($_SERVER['ORIG_PATH_INFO']), PARAM_PATH);
            }
        }
