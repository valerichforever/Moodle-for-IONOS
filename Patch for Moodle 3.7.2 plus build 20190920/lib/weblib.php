...
/**
 * Extracts file argument either from file parameter or ORIG_PATH_INFO
 *
...
function get_file_argument() {
    ...
    if (isset($_SERVER['REQUEST_URI']) && !empty($_SERVER['REQUEST_URI'])) {
        ...
        if ((strpos($_SERVER['REQUEST_URI'], '/pluginfile.php/') !== false)
                && isset($_SERVER['ORIG_PATH_INFO']) && !empty($_SERVER['ORIG_PATH_INFO'])) {
            // Exclude edge cases like '/pluginfile.php/?file='.
            $args = explode('/', ltrim($_SERVER['ORIG_PATH_INFO'], '/'));
    ...
    if (stripos($_SERVER['SERVER_SOFTWARE'], 'iis') !== false) {
        ...
        if (isset($_SERVER['ORIG_PATH_INFO']) and $_SERVER['ORIG_PATH_INFO'] !== '') {
            ...
            if (strpos($_SERVER['ORIG_PATH_INFO'], $SCRIPT) === false) {
                $relativepath = clean_param(urldecode($_SERVER['ORIG_PATH_INFO']), PARAM_PATH);
            }
        }
    } else {
        // All other apache-like servers depend on ORIG_PATH_INFO.
        if (isset($_SERVER['ORIG_PATH_INFO'])) {
            if (isset($_SERVER['SCRIPT_NAME']) and strpos($_SERVER['ORIG_PATH_INFO'], $_SERVER['SCRIPT_NAME']) === 0) {
                $relativepath = substr($_SERVER['ORIG_PATH_INFO'], strlen($_SERVER['SCRIPT_NAME']));
            } else {
                $relativepath = $_SERVER['ORIG_PATH_INFO'];
            }
